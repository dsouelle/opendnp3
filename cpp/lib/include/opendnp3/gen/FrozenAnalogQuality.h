...
#ifndef OPENDNP3_FROZENANALOGQUALITY_H
#define OPENDNP3_FROZENANALOGQUALITY_H
...
/**
  Quality field bitmask for frozen analog values
*/
enum class FrozenAnalogQuality : uint8_t
{
...
struct FrozenAnalogQualitySpec
{
  using enum_type_t = FrozenAnalogQuality;

  static uint8_t to_type(FrozenAnalogQuality arg);
  static FrozenAnalogQuality from_type(uint8_t arg);
  static char const* to_string(FrozenAnalogQuality arg);
  static char const* to_human_string(FrozenAnalogQuality arg);
  static FrozenAnalogQuality from_string(const std::string& arg);
};
